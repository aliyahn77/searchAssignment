...
class VacuumPlanning(Problem):
    ...
    def generateSolution(self):
        ...
        if self.searchType == 'BFS':
            returnedVal = breadth_first_graph_search(self)
            if returnedVal == None:
                print("dear aliyah, handle this")
            else:
    ...
    def actions(self, state):  ##todo:: are the actions not anywhere u can go, regardless of dirt or not
        ...
        if len(possible_neighbors) == 1:
            ...
            for x in possible_neighbors:
                if x[2] == state:
                    ...
                    if current_x > x[2][0]:
                        actions.append('LEFT')
                    elif current_y < x[2][1]:
                        actions.append('UP')
                    elif current_y > x[2][1]:
                        actions.append('DOWN')
                    elif current_x < x[2][0]:
    ...
    def result(self, state, action): ##todo:: do  i need to check if the new state is a wall
...
class Gui(VacuumEnvironment):
    ...
    def setupTestEnvironment(self):
        ...
        self.create_dirts()
        self.stepCount = 0
        self.searchType = 'None'  ##was: self.searchType = None
        self.solution = []
        self.explored = set()
        self.read_env()
    ...
    def display_explored(self, explored):
        ...
        if len(
                self.explored) > 0:  # means we have explored list from previous search. So need to clear their visual fist
    ...
    def step(self):
        ...
        if self.searchType == 'None':
            print("do nothing if search type is none :)")
            return
